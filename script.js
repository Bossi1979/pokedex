async function initOverview() {
    ...
    renderPokemonOverview(cardNoStart, cardNoStop);
}
...
async function renderPokemonOverview(startValue, endValue) {
...
async function scrollDetected() {
    ...
    if (ScrollDownToMoreThan90Percent(scrollDownValue, addidingPokemonStatus )) {
        ...
        await renderPokemonOverview(cardNoStart, cardNoStop);
...
function showSearchResults() {
    ...
    for (let i = 0; i < findingsArray.length; i++) {
        renderPokemonOverview(findingsArray[i], findingsArray[i])
    }
    if (noSearchMatch()){
        showNotFound();
    }
